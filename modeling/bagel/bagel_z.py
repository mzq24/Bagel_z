...
# 假设您的 GNN Encoder 和 Projector 放在这里
from .gnn_encoder import MapGNNEncoder, MLPconnector 
from .modeling_utils import TimestepEmbedder, PositionEmbedding
from modeling.cache_utils.taylorseer import cache_init

from tqdm import tqdm


class BagelzConfig(PretrainedConfig):
    def __init__(
        self,
        visual_gen=True,
        map_und=True,  # <--- 修改：从 visual_und 变为 map_und
        llm_config=None,
        gnn_config=None, # <--- 修改：用 gnn_config 替换 vit_config
        vae_config=None,
        latent_patch_size=2,
        max_latent_size=32,
        gnn_projector_act="gelu_pytorch_tanh", # <--- 新增
        ...
        **kwargs
        ...
        self.map_und = map_und
        self.llm_config = llm_config
        self.gnn_config = gnn_config
        self.vae_config = vae_config
        self.latent_patch_size = latent_patch_size
        self.max_latent_size = max_latent_size
        self.gnn_projector_act = gnn_projector_act
        self.interpolate_pos = interpolate_pos
        self.timestep_shift = timestep_shift


class Bagelz(PreTrainedModel):
    config_class = BagelzConfig
    base_model_prefix = 'bagelz'

    def __init__(self, language_model, config: BagelzConfig):
        ...
        # vvvvvvvvvvvv  修改：将 visual_und 替换为 map_und vvvvvvvvvvvv
        if config.map_und:
            # 初始化GNN模型和投影层
            self.gnn_model = MapGNNEncoder(config.gnn_config) 
            self.gnn_hidden_size = config.gnn_config.hidden_size
            self.gnn_projector = MLPconnector(self.gnn_hidden_size, self.hidden_size, config.gnn_projector_act)
            # 您可以决定是否需要为 GNN 特征添加位置编码
            # self.gnn_pos_embed = PositionEmbedding(...)
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def forward(
        ...
        # vvvvvvvvvvvv  修改：GNN相关输入参数 vvvvvvvvvvvv
        packed_gnn_features: Optional[torch.Tensor] = None,
        packed_gnn_token_indexes: Optional[torch.LongTensor] = None,
        # gnn_token_seqlens: Optional[torch.IntTensor] = None, # 如果需要
        # packed_gnn_position_ids: Optional[torch.LongTensor] = None, # 如果需要
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        # vvvvvvvvvvvv  修改：处理GNN特征 vvvvvvvvvvvv
        if self.config.map_und and packed_gnn_features is not None:
            # 1. 通过 GNN Encoder
            gnn_embeds = self.gnn_model(packed_gnn_features)
            # 2. 通过投影层
            gnn_embeds = self.gnn_projector(gnn_embeds)
            # 3. (可选) 添加位置编码
            # gnn_embeds = gnn_embeds + self.gnn_pos_embed(...)
            # 4. 插入到主序列中
            packed_sequence[packed_gnn_token_indexes] = gnn_embeds
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        extra_inputs = {}
        if self.use_moe:
            # 假设 'und' 模式的 expert 是通用的语言 expert
            # 我们需要为 map token 指定新的 expert
            # 这里我们定义两种类型的token：文本token和地图token
            # 视觉生成token（vae）可以被看作一种特殊的地图token
            
            # 路由到语言专家的token
            lang_expert_token_indexes = packed_text_indexes
            
            # 路由到地图专家的token
            map_expert_token_indexes = packed_gnn_token_indexes
            if packed_vae_token_indexes is not None:
                 map_expert_token_indexes = torch.cat([map_expert_token_indexes, packed_vae_token_indexes], dim=0)

            extra_inputs.update(
                # 假设LLM的MoE层可以接收这样的参数来做路由
                lang_expert_token_indexes=lang_expert_token_indexes,
                map_expert_token_indexes=map_expert_token_indexes,
            )
