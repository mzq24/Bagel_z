...
import sys 
sys.path.append("/media/z/data/mzq/others/LLaVA-NeXT/small_model/SMART")
try:
    from torch_cluster import radius, radius_graph  # type: ignore
except ImportError:
    from torch_cluster_rewrite import radius, radius_graph  # type: ignore
from torch_geometric.data import Batch
from torch_geometric.data import HeteroData
from torch_geometric.utils import dense_to_sparse, subgraph
from smart.layers.attention_layer import AttentionLayer # type: ignore
from smart.layers import MLPLayer   # type: ignore
from smart.layers.fourier_embedding import FourierEmbedding, MLPEmbedding   # type: ignore
from smart.utils import angle_between_2d_vectors, weight_init, wrap_angle   # type: ignore
import pickle
...
class SMARTMapDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        if map_token_traj_path is not None:
            map_token = pickle.load(open(map_token_traj_path, "rb"))
            map_token['traj_src'] = torch.from_numpy(map_token['traj_src']).to(self.data_dtype)
    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        pt_valid_mask = data['pt_token']['pt_valid_mask']   # (N, )
        pt_pred_mask = data['pt_token']['pt_pred_mask']     # (N, )
        pt_target_mask = data['pt_token']['pt_target_mask'] # (N, )
        mask_s = pt_valid_mask

        pos_pt = data['pt_token']['position'][:, :self.input_dim].contiguous()  # (N, 3(x,y,z))
        orient_pt = data['pt_token']['orientation'].contiguous()                # (N, )
        orient_vector_pt = torch.stack([orient_pt.cos(), orient_pt.sin()], dim=-1)
        token_sample_pt = self.map_token['traj_src'].to(pos_pt.device).to(self.data_dtype)  # (1024, 11, 2) (1024, inference_length, (x, y))
        pt_token_emb_src = self.token_emb(token_sample_pt.view(token_sample_pt.shape[0], -1))   # (1024, 128)
        pt_token_emb = pt_token_emb_src[data['pt_token']['token_idx']]      # (2619, 128)
        ...
        token2pl = data[('pt_token', 'to', 'map_polygon')]['edge_index']    # (2, 2619)
        token_light_type = data['map_polygon']['light_type'][token2pl[1]]   # (2619, )  (331, )
